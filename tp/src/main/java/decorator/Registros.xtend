...
class Registros {
	List<RegistroDeBusqueda> registros=new ArrayList<RegistroDeBusqueda>
		...
		def int cantDeRdosPorTerminal(Terminal terminal){
			registros.filter[rdo| rdo.terminal==terminal].map[rdo| rdo.cantDeResultados].fold(0,[cant1,cant2|cant1+cant2])
				
		}
		def HashMap<String,Integer> informeTotalesPorTerminal(){
			val HashMap<String,Integer> rdo=new HashMap<String,Integer>
			registros.map[registros|registros.terminal].toSet.forEach[terminal| rdo.put(terminal.nombre,cantDeRdosPorTerminal(terminal))]
			rdo		
		}
		def List<Integer> parcialesPorTerminal(Terminal terminal){
			registros.filter[rdo|rdo.terminal==terminal].map[rdo| rdo.cantDeResultados].toList
		}
