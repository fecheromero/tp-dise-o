		...
		if (searchBynd(unPunto.id) != null)
			...
			try {
				...
				if (unPunto.servicios.isNullOrEmpty) {
					...
					unPunto.servicios.forEach[servicio|session.save(servicio)]
				}
...
//		unPunto.validate
	...
	def void delete(PuntoDeInteres unPunto) {
		if(searchBynd(unPunto.id) != null) puntos.remove(searchBynd(unPunto.id)) else {
			throw new NoValidoException("El Punto no existe")
		}
	}

	def PuntoDeInteres searchBynd(int id) {
		this.puntos = allInstances.toSet
		this.puntos.forEach[punto|System.out.println(punto.nombre)]
		this.puntos.findFirst[punto|punto.id == id]
	...
	def PuntoDeInteres get(int id) {
		val session = sessionFactory.openSession
		try {
			return session.createCriteria(typeof(PuntoDeInteres)).setFetchMode("Reviews", FetchMode.JOIN).add(
				Restrictions.idEq(id)).uniqueResult() as PuntoDeInteres
		} catch (HibernateException e) {
			throw new RuntimeException(e)
		} finally {
			session.close
