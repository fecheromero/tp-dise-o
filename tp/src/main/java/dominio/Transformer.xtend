...
def HashSet<SucursalBanco> transformarDeJSONaClaseBanco(String listaDeBancosEnJson) {
	...
	var Type tipoListaDeBancosJson = new TypeToken<ArrayList<BancoJSON>>() {}.getType()
...
def SucursalBanco pasarDeBancoJsonASucBanco(BancoJSON unBancoExterno) {
		...
		return new SucursalBanco(servicios, this.direccionDeBanco(unBancoExterno),unBancoExterno.banco+""+unBancoExterno.sucursal)
		}


// Modelado de la direccion de BancoJSON a SucursalBanco
def Direccion direccionDeBanco(BancoJSON unBancoExterno){
var Point coordenadas =new Point (unBancoExterno.x,unBancoExterno.y)
var direccion = new Direccion("", "", #["", ""], coordenadas, "", "", new Comuna("", new Polygon(#[new Point(0, 0), new Point(0, 0)])), "", "", "", "")
return direccion
}

//	Modelado del servicio
def Servicio servicioJSONAServicio(String servicioJSON) {
	var servicio = new Servicio(servicioJSON, horarioBancario())
	return servicio
}


def Horario horarioBancario() {
